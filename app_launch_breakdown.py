...
def calc_main_thread_key_flow_gap(main_thread_trace_output):
	...
	global result_file
	stack = []
	with open(main_thread_trace_output) as rf, open(op.join(op.dirname(main_thread_trace_output), result_file), 'wb+') as wf:
...
def get_sys_key_flow_data(trace_file):
	...
	with open(trace_file) as rf, open(result_file_full_path, 'wb+') as wf:
		wf.write(trace_file+'\n')
		for line in rf:
			if line.startswith('#'):
				continue
			for i, pattern in enumerate(IMPORTANT_SYS_KEY_FLOW_POINT):
				...
				if matcher:
					...
					wf.write(','.join([result, timestamp, '']))
					wf.write('\n')
					break
